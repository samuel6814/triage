...
The chatbot is designed as a \textbf{Flow Management Engine assist}, not as an autonomous discharge or treatment authority. Three design choices enforce that posture throughout Chapter~\ref{ch:method}:

\begin{enumerate}
\item \textbf{Gate before model.} Non-clinical text never receives a colour (Section~\ref{sec:gate}).
\item \textbf{Softmax as a distribution.} Output is \(\hat{\mathbf{y}}\), not a binary ``safe/unsafe'' certificate; low confidence and near ties escalate to nurses (Section~\ref{sec:classification}).
\item \textbf{TEWS remains nurse-owned.} When vitals exist, TEWS and SATS discriminators govern bedside colour; the text path does not silently overwrite them (Section~\ref{sec:tews}).
\end{enumerate}

Pathway cards therefore propose destinations and timers that nurses can accept, modify, or reject. That principle is repeated in the results discussion (Section~\ref{sec:discussion}) and in the research questions on Softmax confidence (RQ7).
...
\begin{itemize}
\item TEWS is a \textbf{weighted sum of six vital features} \(T=\sum_k w_k f_k(v_k)\) with \(w_k=1\) in the standard adult score, then a piecewise colour map \(C_{\mathrm{TEWS}}(T)\).
...
They are complementary, not competitors. At arrival, text may be the only signal; after nurse assessment, TEWS and clinical discriminators remain authoritative. This thesis therefore documents both, implements Softmax fully, and never treats TEWS absence as a chatbot limitation: nurses already compute TEWS when vitals exist.
...
\hline
Input & Six vitals / flags \(v_k\) & English chief complaint \(X\) \\
