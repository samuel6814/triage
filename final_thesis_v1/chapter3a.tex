...
They are complementary, not competitors. At arrival, text may be the only signal; after nurse assessment, TEWS and clinical discriminators remain authoritative. Formally the two paths are independent:
\[
  X \xrightarrow{\mathrm{BioBERT}} C_{\mathrm{NLP}},
  \qquad
  V \xrightarrow{\mathrm{TEWS}} C_{\mathrm{TEWS}}.
\]
Disagreement \(C_{\mathrm{NLP}}\neq C_{\mathrm{TEWS}}\) does not by itself prove the text model wrong; it is a signal that human review is required. This project therefore documents both, implements Softmax fully, and never treats TEWS absence as a chatbot limitation: nurses already compute TEWS when vitals exist.
...
BioBERT reuses BERT's architecture (12 layers, 768 hidden dimensions, 12 attention heads) but continues MLM on biomedical corpora. Fine-tuning on synthetic Triagegeist chief-complaint / ESI-style label pairs adjusts the classification head so \(h_{\mathrm{[CLS]}}\) maps to five acuity levels.

Clinical tokens such as ``headache'', ``feverish'', ``aches'', and ``weak'' benefit from biomedical pre-training that adapts representations to clinical language. After fine-tuning, their joint attention pattern supports acuity estimation for any presenting complaint, not only the illustrative example used in this chapter.
