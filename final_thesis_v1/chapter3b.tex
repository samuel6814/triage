...
\end{equation}
where \(y \in \{1,\ldots,5\}\) is the nurse-labeled acuity level and \(\hat{y}_{y}\) is the Softmax probability the model assigns to that correct level. Over the full training set the same symbol denotes the empirical risk
\begin{equation}
...
The Softmax head predicts Emergency Severity Index (ESI)--style acuity levels \(1\)--\(5\) learned from training and external evaluation labels (Triagegeist, MIMIC-IV-ED, NHAMCS). Those sources are \textbf{not} SATS-labelled: ESI is primarily resource-based, whereas SATS combines vital-sign TEWS with clinical discriminators. Hospital routing at \examplehospital{} nonetheless uses SATS colours, so this project defines a fixed deterministic operational heuristic \(f_{\mathrm{SATS}}\) that bridges the shared five-tier urgency curve to Ghanaian pathway colours:
\begin{equation}
  ...
  \end{cases}
\end{equation}
where \(\hat{c} = \arg\max_c \hat{y}_c\) is the predicted acuity level, \(f_{\mathrm{SATS}}\) is the fixed level-to-colour map, and \(C_{\mathrm{NLP}}\) is the resulting SATS colour from the text path.

While ESI and SATS operate on fundamentally different clinical philosophies (resource allocation versus physiological discriminators), they share an isomorphic five-tier urgency curve. The heuristic \(f_{\mathrm{SATS}}\) leverages this shared architecture to bridge text-based acuity predictions to Ghanaian pathway colours, functioning strictly as an antecedent routing assist rather than a definitive clinical cross-walk, and not as a claim that the training or external datasets are SATS-labelled.
...
\hline
\end{tabular}
\caption{Proposed ESI-style acuity \(\rightarrow\) SATS colour heuristic for text-first routing.}
\label{tab:fsats-esi}
\end{table}
...
\textbf{Lexical child-marker rule.} BioBERT has no age feature, so Pediatrics is never a default destination for adult ASTS colours. Before the pathway card is finalised, a deterministic (non-neural) check runs on the complaint \(X\). Define the marker set
\begin{equation}
...
\begin{aligned}
  ...
  &\xrightarrow[\ref{sec:encoder}--\ref{sec:attention}]{12} h_{\mathrm{[CLS]}} \\
  &\xrightarrow[\ref{sec:classification}]{} \hat{\mathbf{y}}
    \xrightarrow[\ref{sec:fsats}]{} C_{\mathrm{NLP}}
    \xrightarrow[\ref{sec:pathways}]{} P(C_{\mathrm{NLP}}),
\end{aligned}
  \label{eq:e2e-chain}
\end{equation}
where each arrow label points to the section that defines that transformation: \(g\) is the clinical gate, \(\tau(X)\) is tokenisation, \(H^{(0)}\) is the input embedding matrix, \(h_{\mathrm{[CLS]}}\) is the final summary vector, \(\hat{\mathbf{y}}\) is the five-class Softmax distribution, \(C_{\mathrm{NLP}}=f_{\mathrm{SATS}}(\hat{c})\) is the SATS colour, and \(P(C_{\mathrm{NLP}})\) is the pathway card. If \(g(X) < 0.30\), the chain stops after the gate and no colour is assigned.

\begin{enumerate}
\item \textbf{Clinical relevance gate.} Compute \(g(X)\); if \(g(X) < 0.30\), return a request for clinical detail and stop.
...
\begin{tikzpicture}[
  ...
  box/.style={draw=green!60!black, rectangle, fill=green!10, text width=2.5cm, align=center,
    minimum height=1.1cm, rounded corners, font=\small, inner sep=3pt},
  user/.style={box, fill=gray!12},
  api/.style={box, fill=green!18},
  route/.style={box, fill=triageYellow!25, text width=2.65cm},
  ...
  \node[font=\bfseries\small, above=0.55cm of api] {Deployable triage system};
  \node[box, below=1.35cm of api] (card) {Pathway\\card};
  \node[route, right=of card] (dept) {Department\\routing};
  \node[box, fill=gray!8, left=of card] (tews) {Nurse vitals\\(TEWS)};
  \draw[arrow] (pat) -- (ui);
  \draw[arrow] (ui) -- node[above, font=\footnotesize] {JSON} (api);
  \draw[arrow] (api) -- (gate);
  \draw[arrow] (gate) -- (bert);
  \draw[arrow] (bert.south) |- (card.east);
  \draw[arrow] (card) -- (dept);
  \draw[darrow] (tews) -- (card);
  \node[font=\footnotesize\itshape, text=black!65, below=0.5cm of card] {English chief complaint in; acuity, colour, and pathway out};
\end{tikzpicture}}
