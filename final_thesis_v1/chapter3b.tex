...
\begin{equation}
  ...
  \label{eq:tau-map}
\end{equation}
Here \(\tau\) is the \textbf{WordPiece tokenisation map} used by BERT and BioBERT: a \emph{deterministic discrete encoding} that segments the string using a fixed vocabulary and emits token IDs (including special markers). It is \textbf{not} a neural network layer; it does not learn during fine-tuning. Learning begins at the embedding lookup that follows.

\subsection{Why the maximum length is \(M = 128\)}

The project fixes maximum sequence length at \(M = 128\) tokens because that is the BioBERT-base / deployment configuration used for training and inference. The cap has three roles:
\begin{itemize}
\item \textbf{Architecture match.} BioBERT was pre-trained and fine-tuned with this window; changing \(M\) would require a different checkpoint or expensive re-adaptation.
...
\item \textbf{Bound on context.} At most 128 subword positions (including \texttt{[CLS]}, \texttt{[SEP]}, and pads) enter one forward pass; longer free text must be truncated or summarised before submission.
\end{itemize}

\begin{remark}
The fixed length \(M=128\) is therefore both an architecture match and a practical batching choice: BioBERT was trained with this cap, and padding shorter complaints to 128 lets the hospital batch many requests through the same matrix operations at intake.
\end{remark}
...
\end{equation}
\textbf{Rows versus columns.} By convention in this thesis:
\begin{itemize}
...
\end{equation}
which is exactly the cross-entropy loss used in Section~\ref{sec:training-pipeline} when labels are one-hot.
...
\end{equation}
where \(\sigma\) is the logistic sigmoid. That identity shows why binary logistic regression is Softmax with \(K=2\). The five-class triage head is the same mathematical idea generalised from a weighted coin to five mutually exclusive faces.
...
\end{align*}
Level 3 has the largest Categorical probability (\(\hat{y}_3 \approx 0.69\), rounded to 72\% in some deployment logs; Section~\ref{sec:fsats}). The mass on neighbouring Orange (\(0.07\)) and Green (\(0.15+0.05\)) shows residual uncertainty: Softmax does not claim certainty, only a ranked distribution suitable for pathway assist with nurse oversight when confidence is low (RQ7 in Section~\ref{sec:rqs}).
\end{example}

\begin{remark}
Softmax probabilities are calibrated only to the extent that the training distribution matches live intake. High \(\hat{y}_c\) on Triagegeist holdout text is not a clinical guarantee at \examplehospital{}; low-confidence or contradictory presentations remain mandatory nurse review.
\end{remark}
...
\]
as elsewhere in the thesis.

\paragraph{Step 1: Gate.}
Symptom lexicon hits for ``headache'' (\(0.30\)) and ``feverish'' (\(0.25\)), with no greeting penalty, give \(g(X)=0.55\geq 0.35\) (Section~\ref{sec:gate}). Classification proceeds.
...
\[
  ...
  \text{Outpatient urgent / Pediatrics},\;
  \text{actions},\;
  \text{escalation}\bigr)
\]
