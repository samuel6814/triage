...
\]
where \(X\) is the chief complaint, \(\tau\) is tokenisation, \(H^{(0)}\) is the input embedding matrix, \(\hat{\mathbf{y}}\) is the acuity distribution, \(f_{\mathrm{SATS}}\) is the fixed map from acuity level to SATS colour \(C_{\mathrm{NLP}}\), and \(P(C_{\mathrm{NLP}})\) is the pathway card (destination, \(T_{\max}\), actions, escalation), all developed in Chapter~\ref{ch:method},
with clinical gating, Softmax framed as a Categorical distribution over mutually exclusive acuity levels, and department routing tables matched to Ghanaian healthcare facilities.
